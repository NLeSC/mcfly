...
class FindArchitectureBasicSuite(unittest.TestCase):
    ...
    def test_find_best_architecture_with_class_weights(self):
        """Model should not ignore tiny class with huge class weight. Note that this test is very slow, taking up 40% of all mcfly test time."""
        tf.random.set_seed(1)  # Needs tensorflow API v2
        np.random.seed(1)

        X_train, y_train = _create_2_class_labeled_dataset(2, 98)  # very unbalanced
        X_val, y_val = _create_2_class_labeled_dataset(2, 98)
        X_test, y_test = _create_2_class_labeled_dataset(5, 5)
        class_weight = {0: 2, 1: 0.04}

        best_model, _, _, _ = find_architecture.find_best_architecture(
            X_train, y_train, X_val, y_val, verbose=False, subset_size=1000,
            number_of_models=8, nr_epochs=1, model_type='CNN', class_weight=class_weight)
